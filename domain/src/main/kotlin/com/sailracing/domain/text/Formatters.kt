...
/**
 * Pure formatting of domain values for display: locale-independent, big-screen friendly, and shared by
 * every screen the domain is read on - the phone in the boat and the workbench on the desk.
 */
public object Formatters {

    public const val METERS_PER_SECOND_TO_KNOTS: Double = 1.943844
    ...
    public fun timeOfDay(epochMillis: Long, zoneOffsetMillis: Int = java.util.TimeZone.getDefault().getOffset(epochMillis)): String {
    ...
    public fun signedSeconds(seconds: Double): String {
        val rounded = seconds.roundToLong()
        val sign = if (rounded > 0) "+" else if (rounded < 0) "-" else ""
        return "$sign${abs(rounded)} s"
    }

    public fun seconds(seconds: Double): String = "${seconds.roundToLong()} s"

    /** Metres, whole numbers; kilometres with one decimal above 1 km. */
    public fun meters(meters: Double): String =
        if (meters >= 1000.0) String.format(Locale.ROOT, "%.1f km", meters / 1000.0) else "${meters.roundToInt()} m"

    public fun knots(metersPerSecond: Double): String = String.format(Locale.ROOT, "%.1f kn", metersPerSecond * METERS_PER_SECOND_TO_KNOTS)

    public fun knotsValue(metersPerSecond: Double): String = String.format(Locale.ROOT, "%.1f", metersPerSecond * METERS_PER_SECOND_TO_KNOTS)

    public fun knotsToMetersPerSecond(knots: Double): Double = knots / METERS_PER_SECOND_TO_KNOTS

    /** Signed knots with one decimal: "+0.3 kn", "-1.2 kn", "0.0 kn". */
    public fun signedKnots(metersPerSecond: Double): String {
    ...
    public fun degrees(degrees: Double): String = String.format(Locale.ROOT, "%03d°", ((degrees.roundToInt() % 360) + 360) % 360)

    public fun degrees(degrees: Int): String = degrees(degrees.toDouble())

    /** Signed degrees for shifts and errors: "+4°", "-12°", "0°". */
    public fun signedDegrees(degrees: Double): String {
